...
//
// Note that there are objects from other compilation units that call into the
// default (and probably cout) logger objects during static initialization so
// use the Construct On First Use Idiom
// (https://isocpp.org/wiki/faq/ctors#static-init-order-on-first-use) to
// ensure that both loggers are allocated and completely initialized before
...
// Complete the initialization of logging during static initialization. Note:
// only called from coutLoggerInternal() and defaultLoggerInternal().
//
static void maybeInitializeLogging() {
  static bool initialized;
  if (!initialized) {
    initialized = true; // Note: have to do this first.
    initializeLogger(coutLoggerInternal(), "%v");
    initializeLogger(defaultLoggerInternal(), "[%l] %v");
    spdlog::flush_on(spdlog::level::info);
  }
}

// Return the static cout logger, allocating and initializing it on the first
// call.
//
static spdlog::logger& coutLoggerInternal() {
...
// Return the static default logger, allocating and initializing it on the
// first call.
//
static spdlog::logger& defaultLoggerInternal() {
